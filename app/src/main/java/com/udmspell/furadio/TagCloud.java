...
public class TagCloud implements Iterable {
	...
	public TagCloud(List<Tag> tags, int radius, float[] tagColor1, float[] tagColor2, int textSizeMin, int textSizeMax) {
	...
	private void updateAll() {
		...
		for (int j = 0; j < max; j++) {
			...
			float ry1 = (tagCloud.get(j).getLocY()) * cos_mAngleX + tagCloud.get(j).getLocZ() * -sin_mAngleX;
			float rz1 = (tagCloud.get(j).getLocY()) * sin_mAngleX + tagCloud.get(j).getLocZ() * cos_mAngleX;
